...
namespace DisasterRecovery.Pages.TimesheetApproval
{
    public class DetailsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            if (Timesheet.Approved == false)
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    var postTask = httpClient.PostAsJsonAsync<string>($"/api/Timesheet/UpdateTimesheetStatus/{id}", "Viewed");
                    postTask.Wait();
                }
            }
        ...
        public IActionResult OnPostAsync(int id)
        {
            ...
            {
                httpClient.BaseAddress = new System.Uri(apiBaseUrl);
                var postTask = httpClient.PostAsJsonAsync<string>($"/api/Timesheet/UpdateTimesheetStatus/{id}", "Approved");
                postTask.Wait();
                ...
                {
                    ErrorMessage = "We ran into a problem updating this timesheet. Please Contact Administrator for Assistance.";
                    return Page();
                }
            }
